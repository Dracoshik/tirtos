...
#define TASK_FTP_SERVER_STACK_SIZE            (2048)
...
void ftpserver_thread_entry(UArg a0, UArg a1)
{
    ...
    for (;;)
    {
        ...
        snprintf (str, sizeof(str), "%s[%d]", TASK_FTP_SERVER_CLIENT_NAME, (int)client_sock);
...
static void ftpserver_client_thread_entry (UArg a0, UArg a1)
{
    ...
    if (setsockopt(socket, IPPROTO_TCP, TCP_NODELAY, &opt, sizeof(opt)) != SOCKET_ERROR)
    {
