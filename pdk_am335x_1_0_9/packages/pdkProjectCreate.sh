...
# *                    transportsrio / tsip / uart / usb / wdtimer / vps / dcan
...
# *****************************************************************************
soc_list=("AM335x" "AM437x" "AM571x" "AM572x" "AM574x" "K2E" "K2K" "K2H" "K2L" "K2G" "C6678" "C6657" "DRA72x" "DRA75x" "DRA78x" "OMAPL137" "OMAPL138" "C6748")
endian_list=("little" "big")
module_list=("all" "aif2" "bcp" "cppi" "csl" "dfe" "emac" "fatfs" "fm" "fftc" "gpio" "gpmc" "hyplnk" "i2c" "icss_emac" "iqn2" "mcasp" "mcbsp" "mmap" "mmcsd" "nimu" "nimu_icss" "nwal" "osal" "pa" "pcie" "pktlib" "pruss" "qm" "rm" "sa" "serdes_diag" "spi" "srio" "tcp3d" "tfw" "transportqmss" "transportsrio" "tsip" "uart" "usb" "wdtimer" "vps" "dcan")
...
export CCS_INSTALL_PATH=$TOOLS_INSTALL_PATH/ccsv7
...
# Version of CG-Tools
export CGT_VERSION=8.2.2

# Version of CG-ARM-Tools for M4
export CGT_M4_VERSION=16.9.3.LTS

# Version of XDC
export XDC_VERSION=3.50.03.33

# Version of BIOS
export BIOS_VERSION=6.52.00.12

# Version of CG-Tools for ARM
export CGT_VERSION_ARM=GNU_6.3.1:Linaro

# Version of the IPC
export IPC_VERSION=3.47.00.00

# EDMA3 Version 
export EDMA_VERSION=2.12.05.30B

# Version of the PDK
export PDK_VERSION=1.0.9

# Version of the NDK 
export NDK_VERSION=2.26.00.08

# Version of UIA
export UIA_VERSION=2.21.02.07

# Version of CG-Tools for ARM
export CGT_VERSION_ARM9=16.9.3.LTS
...
# export auto create command by default for use with CCSv5 and CCSv6
...
if [ -d ${SDK_INSTALL_PATH}/edma3_* ]; then
	export RTSC_EDMA=";com.ti.sdo.edma3:$EDMA_VERSION"
fi

if [[ "$SOC" != "AM335x" && "$SOC" != "AM437x" ]]; then
if [ -d ${SDK_INSTALL_PATH}/ipc_* ]; then
	export RTSC_IPC=";com.ti.rtsc.IPC:$IPC_VERSION"
fi
fi

if [ -d ${SDK_INSTALL_PATH}/ndk_* ]; then
	export RTSC_NDK=";com.ti.rtsc.NDK:$NDK_VERSION"
fi
if [ -d ${SDK_INSTALL_PATH}/uia_* ]; then
...
if [ "$PROCESSOR" = "dsp" ]; then
    ...
    find \( -iname "*$MODULE*$BOARD*arm*$PROJECT_TYPE*roject.txt" $AMIC110BOARD \) -execdir bash -c 'ccs_create_arm {}' \;
    if [ $(find \( -iname "*$MODULE*$BOARD*arm*$PROJECT_TYPE*roject.txt" $AMIC110BOARD \) | wc -l) -eq 0 ]; then
       echo "No projects detected"
    fi
elif [ "$PROCESSOR" = "m4" ]; then
    find -iname "*$MODULE*$BOARD*m4*$PROJECT_TYPE*roject.txt" -execdir bash -c 'ccs_create_m4 {}' \;
    if [ $(find -iname "*$MODULE*$BOARD*m4*$PROJECT_TYPE*roject.txt" | wc -l) -eq 0 ]; then
       echo "No projects detected"
    fi
else
